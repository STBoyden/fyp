...
var enumsTemplate = template.Must(template.New("").Parse(`// Code generated by go generate; DO NOT EDIT
...
const ({{range $i, $tile := .Tiles }}
	...
	{{$tile.GetName}}_tile{{if (eq $i 0)}} types = iota{{end}}
	{{else if $tile.Variants}}
	{{range $variant := $tile.Variants}}
	{{$tile.GetName}}_{{$variant.GetSuffix}}_tile
	{{end}}
	{{end}}
	{{end}}
)
